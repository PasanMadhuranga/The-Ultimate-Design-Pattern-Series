...
public class SpreadSheet {
  ...
  private CellContextFactory factory;
  ...
  public void setFontFamily(int row, int col, String fontFamily) {
    ...
    var newContext = factory.getContext(fontFamily, currentContext.getFontSize(), currentContext.isBold());
    cells[row][col].setCellContext(newContext);
  }
  ...
  private void generateCells() {
    for (var row = 0; row < MAX_ROWS; row++)
      for (var col = 0; col < MAX_COLS; col++) {
        var cell = new Cell(row, col, factory.getContext(fontFamily, fontSize, isBold));
        cells[row][col] = cell;
      }
  }
